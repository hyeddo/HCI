...
public class tournaPhoto : MonoBehaviour
{
    public Button[] button = new Button[2];
    public Button[] winButton = new Button[2];
    ...
    public void tourna1()
    {
        ...
        firstPhoto = photoArr[1];


        if (GameManager.instance.pickedNum % 2 != 0 && currPhoto == 1)
            {
                photoArr[inx] = photoArr[GameManager.instance.pickedNum];
                inx++;
            }
        ...
        {
            if (button[0].GetComponent<Image>().sprite == photoArr[i].sprite && i != 1)
            {
                photoArr[inx] = photoArr[i];
                inx++;
            }
            else if(button[0].GetComponent<Image>().sprite == firstPhoto.sprite && i == 1)
            {
                photoArr[inx] = firstPhoto;
                inx++;
                i = GameManager.instance.pickedNum;
            }
        }
        ...
        if (currPhoto > GameManager.instance.pickedNum)//다음 사진이 없으면 다음 라운드로 이동
        {
            ...
            if (GameManager.instance.pickedNum % 2 == 0)
                GameManager.instance.pickedNum = GameManager.instance.pickedNum / 2;
            else if (GameManager.instance.pickedNum % 2 != 0)
                GameManager.instance.pickedNum = (GameManager.instance.pickedNum / 2) + 1;
        }

        else if (currPhoto + 1 > GameManager.instance.pickedNum)//다음 사진이 한 장 남았을 때 다음 라운드로 이동
        {
            backupCurNum = currPhoto;
            backupInx = inx;
            backupNum = GameManager.instance.pickedNum;
        ...
        else if (GameManager.instance.pickedNum == 1)
        {
    ...
    public void tourna2()
    {
        ...
        for (int i = 1; i <= GameManager.instance.pickedNum; i++)
        {
            if (button[1].GetComponent<Image>().sprite == photoArr[i].sprite && GameManager.instance.pickedNum == 2)//마지막라운드
            {
                secondPhoto = photoArr[2];
                photoArr[2] = photoArr[1];
                photoArr[1] = secondPhoto;
            }
            else if (button[1].GetComponent<Image>().sprite == photoArr[i].sprite)
            {
                photoArr[inx] = photoArr[i];
                inx++;
            }
